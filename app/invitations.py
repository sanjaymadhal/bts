...
def _format_resend_error(r: httpx.Response) -> str:
    """Return a user-friendly error for Resend rejections."""
    body = (r.text or "")
    lowered = body.lower()
    ...
    if (
        ...
        and "from" in lowered
        and any(token in lowered for token in ["verified", "verify", "domain", "sandbox"])
    ):
        return (
            "Resend rejected the sender address. Use "
            "Trackr <onboarding@resend.dev> (sandbox) or verify your custom domain in Resend."
        )

    return f"Resend rejected: {r.status_code} {body[:200]}"
...
def _normalize_phone(phone: Optional[str]) -> Optional[str]:
    """Normalize phone strings by trimming whitespace and stripping a leading +91 prefix."""
    if phone is None:
        return None
    cleaned = phone.strip()
    if not cleaned:
...
def _send_invite_email(
    *,
    ...
    friendly_message = _format_resend_error(r)
    return (False, friendly_message, "resend")
...
async def create_invitation(
    ...
    if body.phone:
        user_metadata["phone"] = _normalize_phone(body.phone)
    ...
    try:
        ...
        if _get_data(existing) is None:
            ...
            profile_payload = {
                ...
                "assigned_bus_id": body.assigned_bus_id,
                "must_change_password": True,
            }
            try:
    ...
    if (
        ...
        try:
            ...
            except Exception as profile_update_exc:
                _logger.warning(
                    "[invite] failed to persist assigned_bus_id on profile %s: %s",
                    auth_user_id,
                    profile_update_exc,
                )
            _logger.info(
...
def delete_invitation(
    ...
    #    can't sign in. Prefer the stored `accepted_user_id` because
    #    it is the canonical auth.users primary key and avoids the
    #    brittle email-list lookup that can miss or mis-resolve the
    #    record after a partial delete. If the operator hasn't granted
    #    the service role delete permission this raises — we log and
    #    continue, because the invitation + profile are already gone
    #    and the admin should at least see the row leave the list.
    if profile_id:
        try:
        ...
        try:
            # Fallback for legacy rows that do not yet have
            # `accepted_user_id` populated.
