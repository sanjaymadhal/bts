...
def _load_profile(user: CurrentUser, supabase) -> Profile:
    ...
    try:
        res = (
            ...
            .select("id, role, school_id, display_name, phone, linked_student_id, assigned_bus_id, must_change_password")
            .eq("id", user.user_id)
            .maybe_single()
            .execute()
        )
    ...
    if rows is None:
        try:
            rows = _get_data(
                supabase.table("profiles")
                .select("id, role, school_id, display_name, phone, linked_student_id, assigned_bus_id")
                .eq("id", user.user_id)
                .maybe_single()
                .execute()
            )
    ...
    return Profile(
        ...
        assigned_bus_id=rows.get("assigned_bus_id"),
...
def update_me(
    ...
    if body.phone is not None:
        ...
        patch["phone"] = _normalize_phone(body.phone)
    if patch:
        (
            supabase.table("profiles")
            .update(patch)
